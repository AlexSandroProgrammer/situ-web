<?php
$titlePage = "Lista de Aprendices || Ficha Seleccionada";
require_once("../components/sidebar.php");
if (isNotEmpty([$_GET['id_ficha'], $_GET['ruta']])) {
    $id_ficha = $_GET['id_ficha'];
    $ruta_archivo = $_GET['ruta'];
    // arreglo con ids de la consulta
    $array_keys = [1, 2];
    $listaAprendicesLectiva = $connection->prepare("SELECT usuarios.nombres,usuarios.documento,usuarios.apellidos,usuarios.foto_data,usuarios.celular,usuarios.sexo,usuarios.email,usuarios.fecha_registro,usuarios.fecha_nacimiento,usuarios.tipo_convivencia,usuarios.patrocinio,usuarios.empresa_patrocinadora,usuarios.id_ficha,empresas.nombre_empresa,programas_formacion.nombre_programa,tipo_usuario.tipo_usuario,estado_usuario.estado AS estado_aprendiz,estado_se.estado AS nombre_estado_se FROM usuarios LEFT JOIN fichas ON usuarios.id_ficha = fichas.codigoFicha  LEFT JOIN empresas ON usuarios.empresa_patrocinadora = empresas.id_empresa LEFT JOIN tipo_usuario ON usuarios.id_tipo_usuario = tipo_usuario.id LEFT JOIN estados AS estado_usuario ON usuarios.id_estado = estado_usuario.id_estado LEFT JOIN estados AS estado_se ON usuarios.id_estado_se = estado_se.id_estado LEFT JOIN programas_formacion ON fichas.id_programa = programas_formacion.id_programa WHERE usuarios.id_tipo_usuario = :id_tipo_usuario  AND usuarios.id_ficha = :codigo_ficha");
    $listaAprendicesLectiva->bindParam(":id_tipo_usuario", $array_keys[1]);
    $listaAprendicesLectiva->bindParam(":codigo_ficha", $id_ficha);
    $listaAprendicesLectiva->execute();
    $aprendices = $listaAprendicesLectiva->fetchAll(PDO::FETCH_ASSOC);
    if (!$aprendices) {
        showErrorOrSuccessAndRedirect("danger", "Error", "No se encontraron resultados para la ficha seleccionada.", $ruta_archivo);
        exit;
    }
...
?>
                                    ...
                                    <div class="card-body">
                                        <form action="" method="POST" enctype="multipart/form-data" autocomplete="off" name="aprendizMasivoExcel">
                                            ...
                                            <div class="modal-footer">
                                                <a class="btn btn-danger" href="aprendices.php">
                                                    Cancelar
                                                </a>
                                    ...
                                    <div class="card-body">
                                        <form action="" method="POST" enctype="multipart/form-data" autocomplete="off" name="updateImageAprendiz">
                                            <div class=" mb-3">
                                                ...
                                                <input type="hidden" name="ruta" value="aprendices.php">
                                            </div>
                                            <div class="modal-footer">
                                                <a class="btn btn-danger" href="aprendices.php">
                                                    Cancelar
                                    ...
                                    ?>
                                        <tr>
                                            <td>
                                                ...
                                                <a href="aprendices.php?document=<?php echo $aprendiz['documento'] ?>&ruta=" class="btn btn-info mt-2" title="Cambiar Imagen"><i class='bx bx-image-add'></i></a>
                                            </td>
                                            <?php
    ...
    showErrorOrSuccessAndRedirect("error", "Error de ruta", "Debes enviar informacion valida", $ruta_archivo);
}
    ?>